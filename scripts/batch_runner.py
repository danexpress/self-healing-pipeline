...
def trigger_dag_run(offset: int, batch_size: int, dry_run: bool = False) -> dict:
    ...
    cmd = ["airflow", "dags", "trigger", "self_healing_pipeline", "--conf", conf]
...
def main():
    ...
    parser.add_argument(
        "--total", type=int, required=True, help="Total number of records to process"
    )
    parser.add_argument(
        "--batch-size",
        type=int,
    ...
    parser.add_argument(
        "--start", type=int, default=0, help="Starting offset for resume (default: 0)"
    )
    parser.add_argument(
        "--delay",
        type=float,
